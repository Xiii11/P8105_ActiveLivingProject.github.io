...
stacked_data = commuting_sidewalk_df |> 
  select(neighborhood_geo_id, bicycle_percent, car_truck_or_van_percent, 
         public_transportation_percent, walked_percent) |> 
  melt(id.vars = "neighborhood_geo_id", 
       variable.name = "Commute_Mode", 
       value.name = "Percent")

csassocci_chart <- ggplot(stacked_data, aes(y = neighborhood_geo_id, x = Percent, fill = Commute_Mode)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "Commute Mode and Sidewalk Coverage",
       y = "Neighborhood (Geo ID)",
...
osassoci_df = overweight_overall_df |> 
  ...
  select(geography, number, percent, percent_of_land_area, year) |> 
  rename(
    obs_number = number,
    obs_percent = percent
  ) |> 
...
osassoci_plot = ggplot(osassoci_df, aes(x = percent_of_land_area, y = as.numeric(obs_percent), color = year)) +
